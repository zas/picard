...
class Album(MetadataItem):
    def __init__(self, album_id, discid=None, disc_isrcs=None):
        ...
        # live dict tree cuts its resident size dramatically (~75x measured),
        # which matters a lot for large collections (see PICARD-2172).
    ...
    def _release_node_cache(self) -> dict[str, Any] | None:
        ...
        (a live dict tree is ~75x larger). Deserialized on access, which only
        happens when a session is exported.
        """
    ...
    def _submit_disc_isrcs(self):
        ...
        log.debug_if(
            DebugOpt.ISRC,
            "Disc ISRC submit: %d disc ISRCs, %d medium tracks found",
            len(self._disc_isrcs),
            len(medium_tracks),
        )
        for position, track in enumerate(medium_tracks, 1):
            ...
            if disc_isrc not in current_isrcs:
                ...
                log.debug_if(
                    DebugOpt.ISRC,
                    "Disc ISRC: added %s to track %d metadata",
                    disc_isrc,
                    position,
                )
            ...
            if not recording_id:
                log.debug_if(
                    DebugOpt.ISRC,
                    "Disc ISRC: track %d has no recording ID, skipping submission",
                    position,
                )
                continue
            ...
            log.debug_if(
                DebugOpt.ISRC,
                "Disc ISRC: registering %s for track %d (recording %s) for submission",
                disc_isrc,
                position,
                recording_id,
            )
