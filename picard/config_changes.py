...
# Prefixes for auto-detection of upgrade and downgrade hooks
UPGRADE_FUNCTION_PREFIX = 'upgrade_to_v'
DOWNGRADE_FUNCTION_PREFIX = 'downgrade_from_v'
...
#
#
# TO ADD A DOWNGRADE HOOK:
# -----------------------
#
# Add a new method here, named using the following scheme:
# DOWNGRADE_FUNCTION_PREFIX + version with dots replaced by underscores
#
# For example:
# `downgrade_from_v1_0_0dev1()` reverses what `upgrade_to_v1_0_0dev1()` did.
#
# Downgrade hooks are strongly recommended for reversible changes (renames,
# value reversals, format conversions) but not mandatory.
...
def upgrade_to_v3_0_0dev3(config):
    ...
    rename_option(config, 'toolbar_multiselect', 'allow_multi_dirs_selection', BoolOption, False)


def downgrade_from_v3_0_0dev3(config):
    """Revert: option "allow_multi_dirs_selection" back to "toolbar_multiselect"."""
    rename_option(config, 'allow_multi_dirs_selection', 'toolbar_multiselect', BoolOption, False)
...
def downgrade_from_v3_0_0dev7(config):
    """Revert: change theme option DEFAULT back to SYSTEM"""
    if config.setting['ui_theme'] == str(UiTheme.DEFAULT):
        config.setting['ui_theme'] = 'system'


def upgrade_to_v3_0_0dev8(config):
    """Option "dont_write_tags" was renamed to "enable_tag_saving" (value is reversed)."""
    rename_option(config, 'dont_write_tags', 'enable_tag_saving', BoolOption, False, reverse=True)


def downgrade_from_v3_0_0dev8(config):
    """Revert: option "enable_tag_saving" back to "dont_write_tags" (value is reversed)."""
    rename_option(config, 'enable_tag_saving', 'dont_write_tags', BoolOption, True, reverse=True)
